...
function init() {
    ...
    $('.frame-4 p').html('CASH IN ON<br>THE LEADING<br>MARKET AND<br>BUSINESS NEWS');
...
function start() {
    ...
    function featureAnimation() {
        tlFeature.set('.feature', {x:0, y:-72})
                 .set('.pig-lid-image', {transformOrigin:"0% 100%"})
                 .to('.feature', 1, {ease:Power1.easeInOut, y:-216})
                 .to('.feature', 1, {ease:Power1.easeInOut, y:-172, x:-306, delay:3})
                 .to('.pig-lid-image', 1.25, {ease:Power3.easeIn, rotation:50, delay:0.25})
                 .to('.pig-lid-image', 0.75, {y:"+=100", x:"+=20", rotation:110})

                 .to('.feature', 1.5, {ease:Power1.easeInOut, y:102, autoAlpha:0, delay:0.75})
